...
def kuhn_dfs(G,visited,matching,start):
    ...
    visited.add(start)
    for i in G[start]:
        if  matching[i] is None or kuhn_dfs(G,visited,matching,matching[i]):
            matching[i] = start
            matching[start] = i
            return True
    return False
...
def MVC(G):
    ...
    for v in R:
        if v in visited:
            mvc.append(v)
    return mvc, matchings, L, R

def min_edge(G):
    mvc, matchings, L, R = MVC(G)
    edges = []
    s = set()
    for v in L:
        if matchings[v] is not None:
            edges.append((v, matchings[v]))
            s.add(v)
            s.add(matchings[v])
    for v in G:
        if v not in s:
